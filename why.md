...
Of course, this is a rather simple case, but you can probably see where this is going.  One thing to note is that `notExhaustive()` is the function that is called if your pattern-guarded function is *not* augmenting an existing function by that name.
