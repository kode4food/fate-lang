...
exports.lists = nodeunit.testCase({
  "List comprehensions": function (test) {
    ...
    let script1 = `
      [for y in global.yl select y * 2]
    `;

    let script2 = `
      [for y in global.yl where y > 50 select y * 2]
    `;
    ...
    let script4 = `
      {for y in global.yl where y > 50 select y * 2: y * 4}[102]
    `;

    let script5 = `
      {for y in global.yl select (y): y * 2}[51]
    `;

    let script6 = `
      {for y in global.yl select 'val': y}['val']
    `;

    let script7 = `
      [for y in global.yl where y > 50]
    `;

    let script8 = `
      {for name:value in global.zl where name >= 20}
    `;
  ...
  "Object Construction": function (test) {
    ...
    test.equal(evaluate(`
      {global.name + '1': global.value + 1}['hello1']
    `, data), 10);

    test.equal(evaluate(`
      {global.name + '2': global.value + 1}['hello2']
    `, data), 10);
