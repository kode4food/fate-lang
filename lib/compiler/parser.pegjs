{
  ...
  var isFormatter = fate.Runtime.isFormatter;
  var Syntax = fate.Compiler.Syntax;

  function node() {
    var result = Syntax.node.apply(null, arguments);
    result.line = line();
...
channelDeclaration
  = op:When _ signatures_start:signature
    ...
    statements:statementsTail  {
      var signatures = [signatures_start].concat(signatures_cont);
...
importStatement
  = op:From _ path:modulePath __ Import _ imports:moduleItems  {
...
range
  = rangeIds:rangeIdentifiers _ In __ col:expr guard:guard?  {
...
ifStatement
  = op:IfUnless __ expr:expr NL statements:statements tail:elseTail  {
      if ( !op ) {
        return node('if', expr, tail, statements);
      }
      return node('if', expr, statements, tail);
    }
...
multiplicative
  = head:unary
    tail:( _ op:Multiplicative __ r:unary { return node(op, null, r); } )*  {
      return buildBinaryChain(head, tail);
    }
...
memberSelector
  = "." _ elem:Identifier  {
      return node('member', null, elem.template('lit', elem.value));
    }
  / "[" __ elem:expr __ "]"  {
...
objectAssignment
  = name:Identifier PROP_SEP value:expr  {
      return node('objectAssignment', name.template('lit', name.value), value);
    }
  / name:expr PROP_SEP value:expr  {
      return node('objectAssignment', name, value);
    }
  / name:Identifier  {
      return node('objectAssignment', name.template('lit', name.value), name);
    }
  / name:expr  {
      return node('objectAssignment', name);
    }
...
Nil     = "nil"     !IdentCont  { return node('lit', undefined); }
True    = "true"    !IdentCont  { return node('lit', true); }
False   = "false"   !IdentCont  { return node('lit', false); }
...
ReservedWord = ( For / Def / When / From / Import / Export / Let / And / Or /
...
Number
  = c:Card f:Frac? e:Exp?  {
      return node('lit', parseFloat(c + (f ? f : '') + (e ? e : '')));
    }
...
SimpleString
  = '"' '"'                { return node('lit', ''); }
  / "'" "'"                { return node('lit', ''); }
  / '"' c:DoubleChar+ '"'  { return node('lit', c.join('')); }
  / "'" c:SingleChar+ "'"  { return node('lit', c.join('')); }
...
NL = ( WS / Comment )* LFOrEOF ( WS / Comment / LF )*
__ = ( WS / Comment / LF )*
_  = WS*
